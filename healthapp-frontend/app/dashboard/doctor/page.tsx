...
import { Calendar as CalendarIcon, ClipboardCheck, LayoutDashboard, User } from 'lucide-react';
...
export default function DoctorDashboard() {
  ...
  const [selectedPatientProfile, setSelectedPatientProfile] = useState<UserProfileResponse | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    async function fetchPatients() {
      ...
      setAppointments(doctorAppointments);
      const uniquePatients = Array.from(new Set(doctorAppointments.map((a) => a.patientId))).map(
        (id) => ({ id }),
      );
      ...
      <div className="flex flex-1 flex-col">
        ...
        {activeTab === 'Appointments' && (
          <main className="flex gap-6 p-8">
            <div className="flex w-1/3 flex-col rounded-lg bg-white p-4 shadow dark:bg-gray-800">
            ...
            <div className="flex-1 rounded-lg bg-white p-6 shadow dark:bg-gray-800">
              ...
              {activeTab === 'History' && selectedPatientProfile && (
                <div className="space-y-4">
                  ...
                  {appointments.filter((a) => a.patientId === selectedPatientProfile.id).length >
                  0 ? (
              ...
              {activeTab === 'Patients' && selectedPatientProfile && (
                <div className="space-y-2">
                  ...
                  <p>
                    <strong>Phone:</strong> {selectedPatientProfile.phoneNumber || 'N/A'}
                  </p>
                  <p>
                    <strong>Date of Birth:</strong> {selectedPatientProfile.dateOfBirth || 'N/A'}
                  </p>
                  <p>
                    <strong>Address:</strong>{' '}
                    {selectedPatientProfile.address
                      ? `${selectedPatientProfile.address.streetAddress || 'N/A'}, ${selectedPatientProfile.address.city || 'N/A'}, ${selectedPatientProfile.address.state || 'N/A'}, ${selectedPatientProfile.address.postalCode || 'N/A'}, ${selectedPatientProfile.address.country || 'N/A'}`
                      : 'N/A'}
                  </p>
                  <p>
                    <strong>Emergency Contact:</strong>{' '}
                    {selectedPatientProfile.emergencyContact
                      ? `${selectedPatientProfile.emergencyContact.name || 'N/A'} (${selectedPatientProfile.emergencyContact.phoneNumber || 'N/A'})`
                      : 'N/A'}
                  </p>
                </div>
              )}
  ...
  useEffect(() => {
    async function fetchNote() {
      try {
        const text =
          type === 'nurse'
            ? await getNurseNote(appointmentId)
            : await getAppointmentResult(appointmentId);
        setNote(text);
      } catch {
        setNote('N/A');
      }
