...
interface AppointmentWithNotes extends AppointmentResponse {
  nurseNote: string;
  nurseName: string;
  doctorResult: string;
  doctorName: string;
...
export default function DoctorDashboard() {
  ...
  const [patientHistory, setPatientHistory] = useState<AppointmentWithNotes[]>(
    []
  );
        ...
        const appointmentsWithNotes: AppointmentWithNotes[] = await Promise.all(
          patientAppointments.map(async (app) => {
            ...
            try {
              ...
              nurseNoteText = "No note available";
              nurseName = "N/A";
            }
            try {
              ...
              doctorResultText = "No result";
              doctorName = "N/A";
            }
            return {
              ...app,
              nurseNote: nurseNoteText,
              nurseName,
              doctorResult: doctorResultText,
              doctorName,
            };
          })
        );
        appointmentsWithNotes.sort(
          (a, b) =>
            new Date(b.startTime).getTime() - new Date(a.startTime).getTime()
        );
        setPatientHistory(appointmentsWithNotes);
      } catch {
                    ...
                    <ul className="space-y-3">
                      {filtered.map((app) => (
                        ...
                        >
                          ...
                          <p className="text-sm text-gray-500">
                            Patient: {patientNames[app.patientId] || "N/A"}{" "}
                            <br /> Email:{" "}
                            {patientEmails[app.patientId] || "N/A"}
                          </p>
                          <p className="text-sm text-gray-500">
                            {new Date(app.startTime).toLocaleTimeString([], {
                              hour: "2-digit",
